...
//kumahime修改 没有animation的情况下
void MeshAnimation::EvalSubtree(int id)
{
	...
	loopi(0, b.childs.size()) EvalSubtree(b.childs[i]);
}
...
void MeshAnimation::SetBindPose()
{
	...
	{
		loopi(0, bones.size()) bones[i].matrix.ident();
		loopi(0, bones.size()) if (bones[i].parent == -1) EvalSubtree(i);
	}
	//bones[0].matrix.rotate_y(-90. / 180. * 3.14);
	//bones[0].matrix.rotate_x(-100. / 180. * 3.14);
	//[0].matrix.rotate_z(-90. / 180. * 3.14);
	
	//loopi(0, bones[0].childs.size())
	//	EvalSubtree(bones[0].childs[i]);
		
	//loopi(0, 12) bones[0].matrix.rotate_y(-90. / 180. * 3.14);
	//loopi(0, 12) bones[0].matrix.rotate_z(-90. / 180. * 3.14);
